...
fn create_client_config(
    ...
    let conn = state.lock().map_err(|e| e.to_string())?;
    auth_client::create(&conn, &name, &issuer_url, &authorization_url, &token_url, &client_id, &client_secret, &scopes, &grant_type, &extra_params)
}
...
fn update_client_config(
    ...
    let conn = state.lock().map_err(|e| e.to_string())?;
    auth_client::update(&conn, id, &name, &issuer_url, &authorization_url, &token_url, &client_id, &client_secret, &scopes, &grant_type, &extra_params)
}
...
async fn authorize_client(
    ...
    let (grant_type, authorization_url, token_url, client_id, client_secret, scopes, extra_map) = {
        let conn = state.lock().map_err(|e| e.to_string())?;
    ...
    if grant_type == "authorization_code" {
        ...
        let result = auth_client::authorize_code_flow(authorization_url, token_url, client_id, client_secret, scopes, extra_map, cancel_rx).await;
        if let Ok(mut pending_map) = pending_clone.lock() {
            pending_map.remove(&id);
        }
        result
    } else {
        auth_client::authorize_client_credentials(token_url, client_id, client_secret, scopes, extra_map).await
    }
}
