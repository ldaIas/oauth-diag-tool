...
- `src-tauri/` — Rust backend using Tauri 2. Entry point is `main.rs` which calls `lib.rs` for Tauri Builder setup. Initializes logging plugin and SQLite database.
  ...
  - `src-tauri/src/db.rs` — SQLite database initialization with migration system. Four tables: `server_configs`, `auth_server_clients` (with FK cascade delete), `oauth_client_configs`, and `schema_version`. Foreign keys enabled via `PRAGMA foreign_keys = ON`. Versioned migrations run automatically on startup.
  - `src-tauri/src/auth_server.rs` — Server config CRUD and lifecycle (start/stop). Manages `RunningServers` (HashMap of server ID to tokio JoinHandle). Generates URL-safe client IDs (24 chars) and secrets (48 chars). Running servers are stopped on app close via `Drop`.
...
- `ui/` — Elm frontend following The Elm Architecture (Model/Update/View). Compiled JS output goes to `ui/dist/main.js`, served alongside `ui/dist/index.html` and `ui/dist/styles.css`.
  ...
  - `ui/src/ClientConfigList.elm` — client configuration list display. Each config card shows OAuth details (issuer, auth URL, token URL, client ID, secret, scopes, grant type, extra params, disabled params). Tracks per-config auth state (Idle/Loading/Done/Error) and expandable sections for results and parameter toggles. Uses `Action` type for parent communication.
...
**Tauri commands:** `create_server_config`, `get_server_configs`, `delete_server_config`, `add_client_to_server`, `delete_client`, `get_client_configs`, `create_client_config`, `update_client_config`, `delete_client_config`, `start_server`, `stop_server`, `get_callback_url`, `authorize_client`, `cancel_authorization`.

**Frontend serves from** `ui/dist/` (configured as `frontendDist` in `tauri.conf.json`).

**Current state:** Split-panel UI. Left side: server list and create-server form. Right side: client configuration list and create/edit form. Server form collects name/port and auto-derives URLs. Ports auto-assign starting at 9500. Server configs persist in SQLite. Each server supports OAuth clients with auto-generated URL-safe client IDs (24 chars) and secrets (48 chars). Server clients section is expandable (collapsed by default). Clients cascade-delete when server is removed. Client configurations store full OAuth details (issuer, auth URL, token URL, client ID/secret, scopes, grant type, extra params, disabled params for both authorize and token requests) and can be created manually, imported from server clients, or edited. Auth results section on client configs is expandable (collapsed by default, auto-expands on Authorize). OAuth servers can be started/stopped from the UI. Running servers serve both `client_credentials` and `authorization_code` grant types via Axum, validating credentials against the DB in real time. The authorization_code flow includes a consent page (`GET /authorize`) and code exchange (`POST /token`). On the client side, authorization_code triggers a browser-based flow: a callback server is started on fixed port 5757, Chrome opens to a trampoline page that redirects to the authorization URL, and the received code is exchanged for tokens with PKCE (S256). In-flight authorization_code flows can be cancelled and have a 5-minute timeout. Access tokens are opaque UUIDs. All servers are stopped on app close.
