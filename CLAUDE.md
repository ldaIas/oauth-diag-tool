...
- `src-tauri/` — Rust backend using Tauri 2. Entry point is `main.rs` which calls `lib.rs` for Tauri Builder setup. Initializes logging plugin and SQLite database.
  ...
  - `src-tauri/src/db.rs` — SQLite database initialization with migration system. Four tables: `server_configs`, `auth_server_clients` (with FK cascade delete), `oauth_client_configs`, and `schema_version`. Foreign keys enabled via `PRAGMA foreign_keys = ON`. Versioned migrations (currently 9) run automatically on startup. Recent migrations added `redirect_url_override`, `access_token_expiry`, and `refresh_token_expiry` columns to `server_configs`, and `refresh_token` and `disabled_refresh_params` columns to `oauth_client_configs`.
  ...
  - `src-tauri/src/oauth_server.rs` — Axum-based OAuth server. Routes: `GET /authorize` (renders consent page for authorization_code flow), `POST /authorize` (processes approve/deny, issues auth codes, redirects to callback), `POST /token` (client_credentials, authorization_code, and refresh_token grants, validates against DB, returns JWT access tokens signed with HMAC-SHA256), `GET /resource` (validates Bearer JWT from Authorization header, returns decoded claims on success, 401 with WWW-Authenticate header on failure, emits resource-access event to frontend via AppHandle), `GET /.well-known/openid-configuration` (discovery document). Supports both `client_secret_post` and `client_secret_basic` authentication. Auth codes stored in-memory via `Arc<Mutex<HashMap>>`. Refresh tokens issued when scope includes `offline_access`, stored in-memory with configurable expiry. Token expiry (access and refresh) configurable per server via settings.
  - `src-tauri/src/auth_client.rs` — Client-side OAuth flow execution. Handles `client_credentials` (direct token request), `authorization_code` (browser-based flow with PKCE), and `refresh_token` (token refresh using stored refresh token). The auth code flow uses a fixed callback server on port 5757, opens Chrome via a trampoline page, and exchanges the received code for tokens. Supports cancellation of in-flight authorizations and a 5-minute timeout. Individual authorization/token request parameters can be disabled via `disabled_params` and `disabled_token_params`. Refresh tokens from authorization responses are automatically persisted to the database; token rotation is supported (new refresh tokens replace old ones). Also implements `fetch_server_metadata()` which retrieves OAuth server metadata from `/.well-known/oauth-authorization-server`.
...
- `ui/` — Elm frontend following The Elm Architecture (Model/Update/View). Compiled JS output goes to `ui/dist/main.js`, served alongside `ui/dist/index.html` and `ui/dist/styles.css`.
  ...
  - `ui/src/ServerList.elm` — server list display with nested OAuth clients. Each server card shows details, start/stop controls, running status, a resource section (showing the resource endpoint URL, last client that connected, status, and error details — updated in real-time via Tauri events), and a client section with add/delete functionality. Delete is disabled while server is running. Clients have an "Import" button to create a client configuration. Includes an expandable settings section per server for configuring redirect URL override and token expiry values. Uses `Action` type for parent communication.
